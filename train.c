...
void exit_with_help()
{
	...
	"-l reg_weight : set the regulization diagonal matrix (default all 1) \n"
	"   -l 10:0.1,30:0.5  \n"
	"       set 0-10 dim with 0.1, 10-30 dim with 0.5  others are 1  \n"
	...
	for(i=1;i<argc;i++)
	{
		...
		switch(argv[i-1][1])
		{
			...
			case 'l':
				...
				{
					++ param.nr_l;
					param.l_idx = (int *) realloc(param.l_idx,sizeof(int)*param.nr_l);
					param.l_val = (double *) realloc(param.l_val,sizeof(double)*param.nr_l);
					param.l_idx[param.nr_l-1] = atoi(ptr);
					ptr = strchr(ptr,':');
					if (ptr == NULL)
					{
						fprintf(stderr,"regulization weights is not in pair format: 1:100,12:100");
						exit_with_help();
					}
					param.l_val[param.nr_l-1] = atof(ptr+1);
					ptr = strchr(ptr,',');
					if(ptr++ == NULL)
						break;
				}
